...
public class DiscoveryServiceImpl implements DiscoveryService {
    ...
    public DiscoveryServiceImpl(NewsSourceRepository newsSourceRepository,
    ...
    public List<RawIngestion> discoverAndDeduplicate() {
        ...
        log.info("[DiscoveryService] Starting discovery run ID: {}", runId);

        List<NewsSource> activeSources = newsSourceRepository.findByActiveTrue();
        log.info("[DiscoveryService] Found {} active sources", activeSources.size());
        ...
        List<CompletableFuture<Void>> futures = activeSources.stream()
                ...
                .exceptionally(ex -> {
                    log.error("[DiscoveryService] Failed fetching feed {}: {}", source.getName(), ex.getMessage());
                    return null;
                }))
                .toList();

        CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).join();
        log.info("[DiscoveryService] Fetched {} total candidate updates", rawUpdates.size());
        ...
        for (RawUpdateDTO update : rawUpdates) {
            ...
            if (isDuplicate) {
                ...
                log.debug("[DiscoveryService] Duplicate detected for title '{}' (Event ID: {})", cleanedTitle, entity.getEventId());
            } else {
                ...
                log.info("[DiscoveryService] Unique article discovered: '{}' (Event ID: {})", cleanedTitle, entity.getEventId());
            }
        ...
        log.info("[DiscoveryService] Redesign Discovery finished. Total ingested: {}, Unique updates found: {}", 
